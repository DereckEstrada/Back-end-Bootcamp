        ...
        public static Expression<Func<ProductoDTO, bool>> DictionaryProducto(string?opcion, string? data, string? data2)
        {
            ...
            else
            {
                if (validarData2){
                    ...
                    DictionaryProducto.Add("id", x => x.ProdId == in1 && x.Estado == 1);
                    DictionaryProducto.Add("descripcion", x => x.ProdDescripcion.ToLower().Equals(data.ToLower())&& x.Estado == 1);
                    DictionaryProducto.Add("categoria", x => x.CategoriaId== in1 && x.Estado == 1);
                    DictionaryProducto.Add("empresa", x => x.EmpresaId== in1 && x.Estado == 1);
                    DictionaryProducto.Add("proveedor", x => x.ProveedorId== in1 && x.Estado == 1);
                    DictionaryProducto.Add("precio", x => x.ProdUltPrecio== de1 && x.Estado == 1);
                    DictionaryProducto.Add("estado", x => x.Estado== in1);
                ...
                else
                {
                    ...
                    DictionaryProductoDoble.Add("precio", x => (de1<=x.ProdUltPrecio && x.ProdUltPrecio<=de2) && x.Estado==1);
                    DictionaryProductoDoble.Add("fechaact", x => (date1<=x.FechaHoraAct&& x.FechaHoraAct<=date2) && x.Estado==1);
